...
const VERSION_FILE =
  'https://github.com/Arcensoth/mcdata/raw/master/VERSION.txt';
const ITEMS_FILE =
  'https://github.com/Arcensoth/mcdata/raw/master/processed/reports/registries/item/data.json';
...
const main = async () => {
  ...
  const { values: allItems } = (await (
    await fetch(ITEMS_FILE)
  ).json()) as Registry;
  ...
  const diff = allItems.filter(
    (v) => !ids.includes(v) && v !== 'minecraft:air'
  );
